...
const EditUserProfile: React.FC = () => {
  ...
  const handleSave = async () => {
    const userId = await AsyncStorage.getItem('USERID');
    if (userName && email && contactNo && nic && userId) {
      try {
        await firebase.firestore().collection('VehicleOwners').doc(userId).set({
          name: userName,
          email,
          contactNo,
          nic
    ...
    <SafeAreaView>
      <View style={styles.Container}>
        <Appbar.Header style={{ backgroundColor: 'white' }}>
          <Appbar.BackAction color='black' onPress={() => navigation.navigate('userView' as never)} />
          <Appbar.Content title='Profile' color='black' style={{ alignItems: 'center', }} />
